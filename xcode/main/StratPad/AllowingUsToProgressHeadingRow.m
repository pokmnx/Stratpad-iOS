//
//  AllowingUsToProgressHeadingRow.m
//  StratPad
//
...
@interface AllowingUsToProgressHeadingRow (Private)
- (void)calculateLayoutForRow;
- (void)drawBackground;
- (void)drawContent;
@end

@implementation AllowingUsToProgressHeadingRow
...
- (void)calculateLayoutForRow
{
    valueColumnWidth_ = (endingXForValueColumns_ - startingXForValueColumns_) / (columnDates_.count - 1);  //don't render the last quarter date as a column
    
    rowHeight_ = [AllowingUsToProgressHeadingRow heightWithRowWidth:rect_.size.width font:font_ restrictedToHeight:kMaxRowHeight];
}
...
- (void)drawContent
{
    ...
    for (uint i = 0; i < columnDates_.count - 1; i++) {
        ...
        NSDate *quarterDate = [columnDates_ objectAtIndex:i];
