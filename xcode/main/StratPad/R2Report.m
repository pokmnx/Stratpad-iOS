...
{
    NSString *msg = isPrint_ ? LocalizedString(@"R2_REPORT_TITLE_FOR_PRINT", nil) : LocalizedString(@"R2_REPORT_TITLE_FOR_SCREEN", nil);
    NSString *ordKey = [NSString stringWithFormat:@"ORDINAL_%i", pageNumber_];
...
- (void)drawPage:(NSInteger)pageIndex inRect:(CGRect)rect
{
    ...
    // define the layout percentages for print.
    SkinManager *skinMan = [SkinManager sharedManager];
    ...
    [reportHeader addTitleItemWithText:[self companyName] 
                                  font:[UIFont fontWithName:boldFontName_ size:reportHeaderSubTitleFontSizeForPrint] 
                              andColor:reportHeaderFontColorForPrint];
    [reportHeader addTitleItemWithText:[self stratFileName] 
                                  font:[UIFont fontWithName:fontName_ size:reportHeaderSubTitleFontSizeForPrint] 
                              andColor:reportHeaderFontColorForPrint];
    [reportHeader addTitleItemWithText:[[NSDate date] formattedDate2] 
                                  font:[UIFont fontWithName:obliqueFontName_ size:reportHeaderSubTitleFontSizeForPrint] 
                              andColor:reportHeaderFontColorForPrint];
    [reportHeader addDescriptionItemWithText:[NSString stringWithFormat:@"%@ %@", self.reportInfo, LocalizedString(@"ROUNDING_DISCLAIMER_SHORT", nil)] 
                                        font:[UIFont fontWithName:fontName_ size:reportHeaderDescriptionFontSizeForPrint] 
                                    andColor:reportHeaderFontColorForPrint];
...
{
    NSDate *startDate = [calculationStrategy_ reportStartDate];
    ...
    uint year = pageNumber_;
    
    for (uint i = 0; i < 12; i++) { 
        [comps setMonth:i + year*12];
    ...
    NSString *ordKey = [NSString stringWithFormat:@"ORDINAL_ABBR_%i", pageNumber_];
    NSString *ordinal = LocalizedString(ordKey, nil);
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ changeInRevenueForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ changeInRevenueForYear:year]];
    [values addObject:[calculationStrategy_ changeInRevenueForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ changeInCOGSForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ changeInCOGSForYear:year]];
    [values addObject:[calculationStrategy_ changeInCOGSForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ totalChangeInGrossMarginForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ totalChangeInGrossMarginForYear:year]];
    [values addObject:[calculationStrategy_ totalChangeInGrossMarginForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ changeInRadForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ changeInRadForYear:year]];
    [values addObject:[calculationStrategy_ changeInRadForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ changeInGaaForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ changeInGaaForYear:year]];
    [values addObject:[calculationStrategy_ changeInGaaForYearsAfter:year]];
...
- (void)drawChangeInSamRowInRect:(CGRect)rect
{
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ changeInSamForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ changeInSamForYear:year]];
    [values addObject:[calculationStrategy_ changeInSamForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ totalChangeInExpensesForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ totalChangeInExpensesForYear:year]];
    [values addObject:[calculationStrategy_ totalChangeInExpensesForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ netContributionForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ netContributionForYear:year]];
    [values addObject:[calculationStrategy_ netContributionForYearsAfter:year]];
...
{    
    ...
    uint year = pageNumber_;
    for (uint i = 0; i < 12; i++) {
        [values addObject:[calculationStrategy_ netCumulativeForMonthNumber:i+year*12]];
    }
    [values addObject:[calculationStrategy_ netCumulativeForYear:year]];
    [values addObject:[calculationStrategy_ netCumulativeForYearsAfter:year]];
