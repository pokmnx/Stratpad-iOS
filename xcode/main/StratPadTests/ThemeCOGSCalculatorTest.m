...
@interface ThemeCOGSCalculatorTest : SenTestCase {
}
@end


@implementation ThemeCOGSCalculatorTest
 ...
 * only the first month of the theme will contain the one time cogs value.
 */
...
{    
    ...
    theme.cogsOneTime = [NSNumber numberWithDouble:200];
    [stratFile addThemesObject:theme];
    
    ThemeCOGSCalculator *calculator = [[ThemeCOGSCalculator alloc] initWithTheme:theme andIsOptimistic:YES];        
...
{    
    ...
    ThemeCOGSCalculator *calculator = [[ThemeCOGSCalculator alloc] initWithTheme:theme andIsOptimistic:NO];        
 ...
 * every month of the theme will contain the monthly cogs value.
 */
- (void)testCalculateMonthlyValues1
{    
    StratFile *stratFile = [TestSupport createEmptyStratFile];
    Theme *theme = (Theme*)[DataManager createManagedInstance:NSStringFromClass([Theme class])];
    theme.cogsMonthly = [NSNumber numberWithDouble:2000];
    [stratFile addThemesObject:theme];
 ...
 * the quarter cogs value will occur in months 0, 3, 6, ... (i.e., the first month of each quarter)
 */
- (void)testCalculateQuarterlyValues1
{    
    StratFile *stratFile = [TestSupport createEmptyStratFile];
    Theme *theme = (Theme*)[DataManager createManagedInstance:NSStringFromClass([Theme class])];
    theme.cogsQuarterly = [NSNumber numberWithDouble:20000];
    [stratFile addThemesObject:theme];
 ...
 * is expected that the quarter cogs value will occur in months 2 and 5 (i.e., the last month of each quarter)
 */
 ...
 * is expected that the quarter cogs value will occur in months 2, 5, and 7 (i.e., the last month of each of the first two quarters, 
 ...
 * It is expected that the annual cogs value will occur in months 0, 12, ... (i.e., the first month of each year)
 */
- (void)testCalculateAnnualValues1
{    
    StratFile *stratFile = [TestSupport createEmptyStratFile];
    Theme *theme = (Theme*)[DataManager createManagedInstance:NSStringFromClass([Theme class])];
    theme.cogsAnnually = [NSNumber numberWithDouble:200000];
    [stratFile addThemesObject:theme];
 ...
 * is expected that the annual cogs value will occur the first month of each year, and in the last month of the theme.
 */
 ...
 * is expected that the annual cogs value will occur the last month of the theme.
 */
